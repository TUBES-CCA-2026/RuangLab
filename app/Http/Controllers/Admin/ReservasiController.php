...
=======
        ...
        // Cek bentrok dengan yang sudah disetujui
...
=======
    ...
    {
        $request->validate([
            'status'        => ['required', 'in:pending,disetujui,ditolak,sedang_dipakai,hangus,selesai'],
            'catatan_admin' => ['nullable', 'string'],
        ]);

        $reservasi = TrxReservasi::with('user')->findOrFail($id);
        ...
        try {
            if ($reservasi->user) {
               $reservasi->user->notify(new ReservasiStatusChanged(
    $reservasi,
    $request->status,
    'Status reservasi Anda telah diperbarui menjadi ' . ucwords(str_replace('_', ' ', $request->status)) . '.',
    $request->catatan_admin
));
            }
        } catch (\Exception $e) {
            // Notifikasi gagal tidak batalkan aksi
        }
